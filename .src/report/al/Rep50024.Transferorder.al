//Formról indítható verzió
report 50024 "Transfer order SEI"
{
    Caption = 'Transfer order SEI';
    ApplicationArea = All;
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = '.src/report/rdlc/Rep50024.transfer-order.rdlc';
    dataset
    {
        ...
        {
            column(HTransferOrderNo; "No.") { }
            column(HTransferOrder; "Posting Date") { }
            column(HAtadoName; "Transfer-from Name") { }
            column(HAtadoCity; "Transfer-from City") { }
            column(HAtadoCode; "Transfer-from Code") { }
            column(HAtadoPost_Code; "Transfer-from Post Code") { }
            column(HAtadoCounty; "Transfer-from County") { }
            column(HAtvevoName; "Transfer-to Name") { }
            column(HAtvevoCity; "Transfer-to City") { }
            column(HAtvevoPost_Code; "Transfer-to Post Code") { }
            column(HAtvevoCode; "Transfer-to Code") { }
            column(HAtvevoCounty; "Transfer-to County") { }
            column(HIn_Transit_Code; "In-Transit Code") { }
            column(BarCodeTransferOrderNo; CompInfoItemNo2.Picture) { }

            dataitem("Transfer Line"; "Transfer Line")
            {
                DataItemLink = "Document No." = field("No.");
                column(LDocument_No_; "Document No.") { }
                column(LItem_No_; "Item No.") { }
                column(LLine_No_; "Line No.") { }// rdlc guppoláshoz kell, mert a kulcsokat célszerű használni a group-hoz.
                column(LDescription; Description) { }
                column(LQty__to_Ship; "Qty. to Ship") { }
                column(LQtytoReceive; "Qty. to Receive") { }
                column(LQuantity_Shipped; "Quantity Shipped") { }
                column(LUnitofMeasure; "Unit of Measure") { }
                column(LAtadoBinCode; "Transfer-from Bin Code") { }
                column(LAtvevoBinCode; "Transfer-To Bin Code") { }
                column(BarCodeItemNo; CompInfoItemNo.Picture) { }

                trigger OnAfterGetRecord()
                begin
                    CompInfoItemNo.Init();
                    C128MakeBarcode(Format("Item No."), CompInfoItemNo, 3000, 2000, 96, false);
                end;
            } //transferline

            trigger OnPreDataItem()
            begin
                // if RqpItemNumber <> '' then begin
                //     "Transfer Header".SetFilter("No.", '=%1', RqpItemNumber);
                //     //"Transfer Line".SetRange("Document No.", RqpItemNumber);
                // end else begin
                //     //"Transfer Header".SetFilter("No.", '=%1', '1001');
                // end;
            end;
            ...
            begin
                CompInfoItemNo.Init();
                C128MakeBarcode(Format("No."), CompInfoItemNo2, 3000, 2000, 96, false);
            end;
    ...
    labels
    {
        LblTransferingWhse = 'Transfering warehouse';
        LblReceptionWhse = 'Reception warehouse';
        LblTransferWhseCode = 'Transfer whse. code : ';
        LbLTitle = 'Transfer order';
        LblPage = 'Page :';
        LblItemNumber = 'Item number';
        LblItemDescription = 'Description';
        LblItemQty = 'Quantity';
        LblDeliveredItemQty = 'Delivered quantity';
        LblItemUnit = 'Unit of measure';
        LblTransferFromBin = 'Bin code of transf. loc.';
        LblTransferToBin = 'Bin code of rec. loc.';
        LblSingnOfHandover = 'Sign of handover';
        LblSignOfRecipient = 'Sign of recipient';
        LblDate = 'Date';
    }
    ...
    var
        ...
        CompInfoItemNo: Record "Company Information";
        CompInfoItemNo2: Record "Company Information";
